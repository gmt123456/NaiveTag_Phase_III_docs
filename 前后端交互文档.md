...
| ------ | ----- | ------------------------------- |
| 廖均达、訾源 | 初稿    | 初稿                              |
| 訾源     | 5月19日 | 修改worker部分访问传递参数细节，将获取用户信息分成两部分 |
|        |       |                                 |
...
```
前端访问url：/worker/userInfo/editUserName.html

Get
token
userName
返回
state: "xxx" // 可选：success, fail(不知道为什么失败了)

前端访问url：/worker/userInfo/editAvatar.html
